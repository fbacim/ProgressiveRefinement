...
class SQUADBubblev1(PointingTechnique):
	...
	def createSQUADQuadrants(self,list,centered=0):
		...
		# first thing is to sort the list based on selected quadrant and distance
		'''
		self.selectedQuadrant
		newList = []
		numObjectsInQuad = math.floor(float(len(list))/4.0)
		index = 0
		for clone in list:
			clone
			index += 1
	...
	def calculateSphereCastingFrustum(self):
		...
		ratio = viz.MainWindow.getAspectRatio()
	...
	def update(self):
		...
		if self.mode == InteractionMode.SELECTION:
			...
			for obj in objects_in_cone:#self.objects:
				...
				if Vector3.PositionInRay(origin,direction,point) > 0:
					obj_size = obj.getBoundingBox(viz.ABS_GLOBAL).size
					if obj_size[0] < 40 and obj_size[1] < 40 and obj_size[2] < 40:
						...
						#if distance < min_distance and Vector3.PositionInRay(origin,direction,point) > 0:
