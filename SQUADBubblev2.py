	...
	def selectSQUADQuadrant(self):
		...
		if len(self.clone_list) > 0:
			...
			if len(new_clone_list) > 1:
				...
				# we have to calculate distances for the new list
				...
				# first thing is calculate how many objects will be in next quadrant
				max_per_quad = int(math.ceil(float(len(new_clone_list))/4.0))
				#print max_per_quad
				
				# then, find the threshold
				#print self.distance_list
				distance_list_sorted = sorted(self.distance_list)
				distance_threshold = distance_list_sorted[max_per_quad-1]
				#print distance_list_sorted
				
				# now, we use the distance_threshold and selectedQuad to sort the list
				#print self.distance_list
				#print new_clone_list 
				#print distance_threshold
				
				sorted_list = []
				list_size = len(new_clone_list)
				quadrant = 0
				for i in range(list_size):
					#print i, quadrant, self.selectedQuadrant
					#print "  ", self.distance_list
					#print "  ", new_clone_list
					#print "  ", sorted_list
					if quadrant == self.selectedQuadrant:
						index = 0
						...
						sorted_list.append(new_clone_list.pop(index))
					else:
						index = 0
						for d in self.distance_list:
							if d > distance_threshold:
