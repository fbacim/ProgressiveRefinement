...
class ExpandBubblev2(PointingTechnique):
	...
	def update(self):
		...
		if self.mode == InteractionMode.SELECTION:			
			...
			if self.intersecting == True:
				...
				index = 0
				for child in objects_in_cone: #self.objects:
					#if not viz.MainWindow.isCulled(obj):
					obj_size = obj.getBoundingBox(viz.ABS_GLOBAL).size#child.getBoundingBox(viz.ABS_GLOBAL).size
					...
					if obj_size[0] < 40 and obj_size[1] < 40 and obj_size[2] < 40:
						...
						if distance < radiusSquared:
							self.intersectingObjects.append(child)
					index+=1
	...
	def drawIntersectObjsBoundingBoxes(self):
		if self.mode == InteractionMode.SELECTION:
			...
			#for object in self.boundingBoxes:
