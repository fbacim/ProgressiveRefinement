...
class CombinedZoomv4(ZoomTechnique):
	...
	def update(self):
		...
		self.intersectingObjects = self.raycasting.intersectingObjects
		self.selectedObject = self.raycasting.selectedObject
		...
		if self.zoomingLevel < self.maxMultiplier:
			...
			possiblenewoffset = [(float(x)*float(self.windowSize[0])/2.0)/float(self.windowSize[0]), 
								 (float(y)*float(self.windowSize[1])/2.0)/float(self.windowSize[1])]
		...
		if self.buttonState[1] == True and self.zoomingAnimation == 0:
			self.zoomIn()
		elif self.buttonState[2] == True and self.zoomingAnimation == 0:
			self.zoomOut()
		...
		self.userdata = self.zoomingLevel
	...
	def clean(self):
		...
		self.raycasting.selectedObject = None
	...
	def zoomIn(self):
		...
		if self.zoomingLevel*2.0 > self.maxMultiplier:
			...
			self.newCenterOffset = [self.previousCenterOffset[0]+(x*self.windowSize[0]/(self.zoomingLevel*2.0))/self.windowSize[0], 
							        self.previousCenterOffset[1]+(y*self.windowSize[1]/(self.zoomingLevel*2.0))/self.windowSize[1]]
			self.zooming = True
