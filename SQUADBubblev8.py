...
def sphereInFrustum(frustum, center, radius):
...
def nodesInFrustumProcess(args):#objects,bs,Frustum,r):
	'''objects_in_cone = []
	objects_positions_in_cone = []
	index = range[0]
	for obj in objects[r[0]:r[1]]:
		object_position = Vector3(bs[index].center[0],bs[index].center[1],bs[index].center[2])
		object_size = bs[index].radius
		
		if sphereInFrustum(Frustum,object_position,object_size) and object_size < 40:
			# add to auxiliary lists
			objects_in_cone.append(obj)
			objects_positions_in_cone.append(object_position)
		index += 1
	return [objects_in_cone,objects_positions_in_cone]'''
	return 0
...
class SQUADBubblev8(PointingTechnique):
	def __init__(self,sceneObjects,useOctree=None):
		...
		for obj in self.objects:
			...
			self.initialPositions[obj] = Vector3(self.bs[-1].center[0],self.bs[-1].center[1],self.bs[-1].center[2])
		...
		self.objectMap = {(0,0,0,255):[-1,None]}
		...
		for obj in self.objects:
			...
			self.objectMap[hex(compositeColor)] = [index,obj]
			#self.objectMap[(r,g,b,a)] = [index,obj]
			index += 1
		...
		self.debugWindow = viz.addTexQuad(parent = viz.SCREEN)
		self.debugWindow.setPosition(0.5,0.5)
		self.debugWindow.setScale(10,10,1)
	...
	def nodesInFrustumIterative(self,objects_in_cone,objects_positions_in_cone):
		...
		for i in range(processes):
			range_end = range_start + math.floor(len(self.objects)/processes) + (1 if (i < (len(self.objects) % processes)) else 0)
			print 'starting process for range [',range_start,',',range_end,')'
			args.append((self.objects,self.bs,Frustum,[range_start,range_end],))
			#r.append(self.pool.apply_async(nodesInFrustumProcess, args = (self.objects,self.bs,Frustum,[range_start,range_end], )))
			range_start = range_end
		
		results = self.pool.map(nodesInFrustumProcess, args)
		print results
		#for i in range(processes):
		#	res = r[i].get()
		#	objects_in_cone.append(res[0])
		#	objects_positions_in_cone.append(res[1])
		
		'''
		index = 0
		for obj in self.objects:
			object_position = Vector3(self.bs[index].center[0],self.bs[index].center[1],self.bs[index].center[2])
			object_size = self.bs[index].radius
			
			if self.sphereInFrustum(Frustum,object_position,object_size) and object_size < 40:
				# add to auxiliary lists
				objects_in_cone.append(obj)
				objects_positions_in_cone.append(object_position)
			index += 1
	
	def calculateDistancesOctree(self,objects_in_cone,objects_bs_in_cone,min_distance_squared,min_distance_obj,min_distance_point,min_distance_to_point):
		...
		for obj in objects_in_cone:
			...
			if distance < min_distance_squared[0] and objects_bs_in_cone[index].radius < 40:
				min_distance_squared[0] = distance #distance squared
				min_distance_obj[0] = obj
				min_distance_point[0] = point
				distance_to_point = user_position_v3.distanceSquared(point)
				min_distance_to_point[0] = distance_to_point
			index += 1
	
	def calculateDistancesIterative(self,objects_in_cone,objects_positions_in_cone,min_distance_squared,min_distance_obj,min_distance_point,min_distance_to_point):
		user_position = viz.MainView.getMatrix(viz.ABS_GLOBAL)
		user_position_v3 = Vector3(user_position[12],user_position[13],user_position[14])
		x,y = self.cursorPosition[0],self.cursorPosition[1]
		distance_to_point = 0
		index = 0
		for obj in objects_in_cone:
			point = self.initialPositions[obj]#objects_positions_in_cone[index]
			screen_pos = viz.MainWindow.worldToScreen(point[0],point[1],point[2],mode=viz.WINDOW_PIXELS)
			# calculate distance from line to object
			distance = math.pow(screen_pos[0]-x,2)+math.pow(screen_pos[1]-y,2) # distance is squared
			if distance < min_distance_squared[0]:
				min_distance_squared[0] = distance #distance squared
				min_distance_obj[0] = obj
				min_distance_point[0] = point
				distance_to_point = user_position_v3.distanceSquared(point)
	...
	def calculateIntersectionsOctree(self,objects_in_cone,objects_bs_in_cone,origin,direction,min_distance_squared,min_pos_in_ray):
		index = 0
		for obj in objects_in_cone: 
	...
	def calculateIntersectionsIterative(self,objects_in_cone,objects_positions_in_cone,origin,direction,min_distance_squared,min_pos_in_ray):
		index = 0
		for obj in objects_in_cone: 
			point = self.initialPositions[obj]#objects_positions_in_cone[index]
			pos_in_ray = Vector3.PositionInRay(origin,direction,point)
	...
	def update(self):
		...
		self.pointerRadiusUniform.set(20)
		print x,y
		distance = math.sqrt(math.pow(400-x,2) + math.pow(300-y,2))
		print distance
		
		# Set FOV for video camera
		#print viz.MainWindow.getHorizontalFOV(), viz.MainWindow.getAspectRatio()
		...
		if self.mode == InteractionMode.SELECTION:			
			...
			print timer.elapsed()
			print 'Initial list size:',len(self.objects)
			
			if self.useOctree:
				self.nodesInFrustumOctree(objects_in_cone,objects_bs_in_cone)
			else:
				
				#self.nodesInFrustumIterative(objects_in_cone,objects_positions_in_cone)
				'''
				colors = set()
				self.colorTexture.save('image-test.png')
				imageData = self.colorTexture.saveToBuffer('<raw>')
				#im = Image.fromstring('L', [self.colorTexture.getSize()[0],self.colorTexture.getSize()[1]], imageData, 'raw', 'RGBX')
				#by_color = defaultdict(int)
				for i in range(0,self.colorTexture.getSize()[0]*self.colorTexture.getSize()[1]*4,4):
					r = ord(imageData[i])#int(imageData[i].encode('hex'), 16)
					g = ord(imageData[i+1])#int(imageData[i+1].encode('hex'), 16)
					b = ord(imageData[i+2])#int(imageData[i+2].encode('hex'), 16)
					#by_color[(hex(r),hex(g),hex(b),0xff)] += 1
					rgb = (r << 16) | (g << 8) | b
					#by_color[rgb] += 1
					colors.add(hex(rgb))

				#colors.remove(hex(0))
				#colors.remove(hex(16777215))
				print len(colors)#,by_color
				
				for key in colors:
					map_list = self.objectMap[key]
					index = map_list[0]
					objects_positions_in_cone.append(Vector3(self.bs[index].center[0],self.bs[index].center[1],self.bs[index].center[2]))
					objects_in_cone.append(map_list[1])
				
				print 'objects found:',len(colors)
				'''
				xmin = (x-self.min_distance)/self.windowSize[0] if x-self.min_distance >= 0 else 0
				ymin = (y-self.min_distance)/self.windowSize[1] if y-self.min_distance >= 0 else 0
				xmax = (x+self.min_distance)/self.windowSize[0] if x+self.min_distance < self.windowSize[0] else 1.0
				ymax = (y+self.min_distance)/self.windowSize[1] if y+self.min_distance < self.windowSize[1] else 1.0
				print [xmin,ymin,xmax,ymax]
				
				objects_in_cone = viz.MainWindow.pickRect([xmin,ymin,xmax,ymax],viz.WORLD)
				#objects = set()
				#for h in frange(xmin,xmax,1.0/self.windowSize[0]):
				#	for v in frange(ymin,ymax,1.0/self.windowSize[1]):
				#		objects.add(viz.pick(pos=[h,v],all=False))
				#objects_in_cone = list(objects)
				#objects_in_cone.remove(viz.VizNode(-1))#'''
			
			print timer.elapsed()
			print 'Objects in frustum:',len(objects_in_cone)
			...
			min_distance_point = [[0,0,0]]
			min_pos_in_ray = [0]
			
			# SCREEN SPACE CALCULATIONS
			'''if self.useOctree:
				self.calculateDistancesOctree(objects_in_cone,objects_bs_in_cone,min_distance_squared,min_distance_obj,min_distance_point,min_distance_to_point)
			else:
				self.calculateDistancesIterative(objects_in_cone,objects_positions_in_cone,min_distance_squared,min_distance_obj,min_distance_point,min_distance_to_point)
			
			min_distance_to_point = min_distance_to_point[0]
			min_distance_obj = min_distance_obj[0]
			min_distance_point = min_distance_point[0]
			min_pos_in_ray = min_pos_in_ray[0]
			min_distance_squared = min_distance_squared[0]
			...
			if min_distance_obj != None:
				...
				self.ccCone.disable(viz.PICKING)
			...
			if self.intersecting == True:
				...
				#print timer.elapsed()
				if self.useOctree:
					self.calculateIntersectionsOctree(objects_in_cone,objects_bs_in_cone,origin,direction,min_distance_squared,min_pos_in_ray)
				else:
					self.calculateIntersectionsIterative(objects_in_cone,objects_positions_in_cone,origin,direction,min_distance_squared,min_pos_in_ray)
			...
			#	bb.renderToAllRenderNodesExcept([self.renderNode])
