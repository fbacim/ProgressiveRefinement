...
class ExpandBubblev5(PointingTechnique):
	def __init__(self,sceneObjects):
		...
		self.ccCone = viz.addTexQuad(viz.SCREEN,1)
	...
	def update(self):
		...
		if self.mode == InteractionMode.SELECTION:			
			...
			if min_distance_obj != None:
				...
				if min_distance_squared < 1600:
					min_distance_squared = 1600 # 20^2 -> this should be set in clas 
				min_distance = min_distance_squared**0.5 # distance squared
				self.ccCone.setPosition(float(x)/float(self.windowSize[0]),float(y)/float(self.windowSize[1]),0)
				self.ccCone.setScale(min_distance/32,min_distance/32)
